...
@implementation ImageShowScrollView
{
...
- (id)initWithFrame:(CGRect)frame withSourceData:(NSMutableArray *)imgSource withIndex:(NSInteger)index placeholderImage:(UIImage *)placeholderImage{
    ...
    if (self) {
        ...
        if (self.placeholderImage == nil) {
            self.placeholderImage = [UIImage imageNamed:@"Image_default"];
        }
...
- (void)setAllImgVContentFromImage:(NSArray *)imgList{
    ...
    if (imgList.count) {
        if ([imgList[0] isKindOfClass:urlCls]) {
            [vLift sd_setImageWithURL:imgList[MRImgLocationLeft]placeholderImage:self.placeholderImage];
            [vCenter sd_setImageWithURL:imgList[MRImgLocationCenter]placeholderImage:self.placeholderImage];
            [vRight sd_setImageWithURL:imgList[MRImgLocationRight]placeholderImage:self.placeholderImage];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    //显示图片对应下标
    HGImageShowViewController *viewController = (HGImageShowViewController *)self.viewController;
    viewController.indexLabel.text = [NSString stringWithFormat:@"%zi/%zi",_curIndex+1,_imgSource.count];
