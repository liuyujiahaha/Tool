...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    if (![messages isKindOfClass:[NSArray class]]) {
        NSLog(@"LvUJsBridge: WARNING: Invalid %@ received: %@", [messages class], messages);
        return;
    }
    for (WVJBMessage* message in messages) {
        if (![message isKindOfClass:[WVJBMessage class]]) {
            NSLog(@"LvUJsBridge: WARNING: Invalid %@ received: %@", [message class], message);
            continue;
        }
...
-(void)logUnkownMessage:(NSURL*)url {
    NSLog(@"LvUJsBridge: WARNING: Received unknown LvUJsBridge command %@://%@", kCustomProtocolScheme, [url path]);
}

-(NSString *)webViewJavascriptCheckCommand {
    return @"typeof LvUJsBridge == \'object\';";
}

-(NSString *)webViewJavascriptFetchQueyCommand {
    return @"LvUJsBridge._fetchQueue();";
}
...
- (void)_dispatchMessage:(WVJBMessage*)message {
    ...
    NSString* javascriptCommand = [NSString stringWithFormat:@"LvUJsBridge._handleMessageFromNative('%@');", messageJSON];
