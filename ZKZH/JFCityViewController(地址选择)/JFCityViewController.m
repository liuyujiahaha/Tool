...
- (void)viewDidLoad {
    ...
    if (@available(iOS 11.0, *)) {
        self.rootTableView.contentInset = UIEdgeInsetsMake(25, 0, 0, 0);
    }
...
- (void)setNaviView
{
    ...
    UILabel *titleLabel = [PublicFunction getlabel:CGRectMake(0, 20, ScreenW, 40) text:iStoreLocalizedString(@"选择城市") fontSize:17 color:[UIColor whiteColor] align:@"center"];
    [view addSubview:titleLabel];
    
    UIButton *backButton = [PublicFunction getButtonInControl:self frame:CGRectMake(0, 20, 44, 44) imageName:@"login_Backup" title:nil clickAction:@selector(back)];
...
- (void)chooseCityWithName:(NSNotification *)info {
    ...
    if ([[cityDic valueForKey:@"cityName"] isEqualToString:iStoreLocalizedString(@"全城")]) {
        __weak typeof(self) weakSelf = self;
...
- (NSMutableArray *)areaMutableArray {
    if (!_areaMutableArray) {
        _areaMutableArray = [NSMutableArray arrayWithObject:iStoreLocalizedString(@"全城")];
    }
...
- (JFCityHeaderView *)headerView {
    if (!_headerView) {
        ...
        _headerView.buttonTitle = iStoreLocalizedString(@"选择区县");
...
-(void)processData:(void (^) (id))success {
    ...
    success(iStoreLocalizedString(@"成功"));
}
...
- (UITableView *)rootTableView {
    if (!_rootTableView) {
        _rootTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 44, ScreenW, ScreenH - 44) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < _HeaderSectionTotal) {
        ...
        if (indexPath.section == _HeaderSectionTotal - 3) {
            ...
            _cell.cityNameArray = locationCity ? @[locationCity] : @[iStoreLocalizedString(@"正在定位...")];
        }
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if (_HeaderSectionTotal == 3) {
        switch (section) {
            case 0:
                return iStoreLocalizedString(@"定位城市");
                break;
            case 1:
                return iStoreLocalizedString(@"最近访问的城市");
                break;
            case 2:
                return iStoreLocalizedString(@"热门城市");
                break;
            default:
