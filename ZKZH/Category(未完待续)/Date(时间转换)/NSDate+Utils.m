...
- (NSString *)weekday
{
    ...
    switch (weekday) {
        case 1:
            week = iStoreLocalizedString(@"星期日");
            break;
        case 2:
            week = iStoreLocalizedString(@"星期一");
            break;
        case 3:
            week = iStoreLocalizedString(@"星期二");
            break;
        case 4:
            week = iStoreLocalizedString(@"星期三");
            break;
        case 5:
            week = iStoreLocalizedString(@"星期四");
            break;
        case 6:
            week = iStoreLocalizedString(@"星期五");
            break;
        case 7:
            week = iStoreLocalizedString(@"星期六");
            break;
...
- (NSString *)timeHourMinuteWithPrefix:(BOOL)enablePrefix suffix:(BOOL)enableSuffix
{
    ...
    if (enablePrefix) {
        timeStr = [NSString stringWithFormat:@"%@%@",([self hour] > 12 ? iStoreLocalizedString(@"下午") : iStoreLocalizedString(@"上午")),timeStr];
    }
    if (enableSuffix) {
...
- (NSString *)stringYearMonthDayCompareToday
{
    ...
    if (chaDay == 0) {
        str = iStoreLocalizedString(@"今天");
    }else if (chaDay == 1){
        str = iStoreLocalizedString(@"明天");
    }else if (chaDay == -1){
        str = iStoreLocalizedString(@"昨天");
    }else{
...
+ (NSString *)dateMonthDayWithDate:(NSDate *)date
{
    ...
    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
    [formatter setDateFormat:iStoreLocalizedString(@"yyyy年MM月dd日")];
    NSString *str = [formatter stringFromDate:date];
    return str;
}
