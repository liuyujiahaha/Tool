...
- (NSString *)barTitle {
    if (!_barTitle) {
        if ([self.creationDate isToday]) {
            _barTitle = iStoreLocalizedString(@"今天");
        }else if ([self.creationDate isYesterday]) {
            _barTitle = iStoreLocalizedString(@"昨天");
        }else if ([self.creationDate isSameWeek]) {
            _barTitle = [self.creationDate getNowWeekday];
        }else if ([self.creationDate isThisYear]) {
            _barTitle = [NSString stringWithFormat:@"%@ %@",[self.creationDate dateStringWithFormat:iStoreLocalizedString(@"MM月dd日")],[self.creationDate getNowWeekday]];
        }else {
            _barTitle = [self.creationDate dateStringWithFormat:iStoreLocalizedString(@"yyyy年MM月dd日")];
        }
    }
    return _barTitle;
}
...
- (NSString *)dateString {
    if (!_dateString) {
        ...
        if ([self.date isToday]) {
            _dateString = iStoreLocalizedString(@"今天");
        }else if ([self.date isYesterday]) {
            _dateString = iStoreLocalizedString(@"昨天");
        }else if ([self.date isSameWeek]) {
            _dateString = [self.date getNowWeekday];
        }else if ([self.date isThisYear]) {
            _dateString = [NSString stringWithFormat:@"%@ %@",[self.date dateStringWithFormat:iStoreLocalizedString(@"MM月dd日")],[self.date getNowWeekday]];
        }else {
            _dateString = [self.date dateStringWithFormat:iStoreLocalizedString(@"yyyy年MM月dd日")];
        }
    }
    return _dateString;
}
