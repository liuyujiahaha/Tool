...
- (void)cellDidDeleteClcik:(UICollectionViewCell *)cell {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:iStoreLocalizedString(@"确定要删除吗？") message:nil preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancel = [UIAlertAction actionWithTitle:iStoreLocalizedString(@"点错了") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
    }];
    UIAlertAction *corfirm = [UIAlertAction actionWithTitle:iStoreLocalizedString(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    [alert addAction:corfirm];
    [[ToolsObject appDelegate].window.rootViewController presentViewController:alert animated:YES completion:nil];
}
