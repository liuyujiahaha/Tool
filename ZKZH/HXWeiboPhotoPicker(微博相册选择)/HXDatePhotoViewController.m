...
- (void)viewDidLoad {
    ...
    [self.view showLoadingHUDText:iStoreLocalizedString(@"加载中")];
    [self getPhotoList];
...
- (void)setupUI {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:iStoreLocalizedString(@"取消") style:UIBarButtonItemStyleDone target:self action:@selector(didCancelClick)];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        ...
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (granted) {
                    ...
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"无法使用相机")] message:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"请在设置-隐私-相机中允许访问相机")] preferredStyle:UIAlertControllerStyleAlert];
                    [alert addAction:[UIAlertAction actionWithTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"取消")] style:UIAlertActionStyleDefault handler:nil]];
                    [alert addAction:[UIAlertAction actionWithTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"设置")] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (cell.model.isIcloud) {
            [self.view showImageHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"尚未从iCloud上下载，请至相册下载完毕后选择")]];
            return;
        }
...
- (void)setVideoCount:(NSInteger)videoCount {
    ...
    if (self.photoCount > 0 && videoCount > 0) {
        self.titleLb.text = [NSString stringWithFormat:@"%ld %@、%ld %@",self.photoCount,iStoreLocalizedString(@"张照片"),videoCount,iStoreLocalizedString(@"个视频")];
    }else if (self.photoCount > 0) {
        self.titleLb.text = [NSString stringWithFormat:@"%ld %@",self.photoCount,iStoreLocalizedString(@"张照片")];
    }else {
        self.titleLb.text = [NSString stringWithFormat:@"%ld %@",videoCount,iStoreLocalizedString(@"个视频")];
    }
}
- (void)layoutSubviews {
    [super layoutSubviews];
...
- (void)setSelectCount:(NSInteger)selectCount {
    ...
    if (selectCount <= 0) {
        ...
        [self.doneBtn setTitle:iStoreLocalizedString(@"完成") forState:UIControlStateNormal];
    }else {
        self.previewBtn.enabled = YES;
        self.doneBtn.enabled = YES;
        [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld)",iStoreLocalizedString(@"完成"),selectCount] forState:UIControlStateNormal];
    }
...
- (UIButton *)previewBtn {
    if (!_previewBtn) {
        ...
        [_previewBtn setTitle:iStoreLocalizedString(@"预览") forState:UIControlStateNormal];
...
- (UIButton *)doneBtn {
    if (!_doneBtn) {
        ...
        [_doneBtn setTitle:iStoreLocalizedString(@"完成") forState:UIControlStateNormal];
