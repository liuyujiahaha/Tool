...
- (UILabel *)authorizationLb {
    if (!_authorizationLb) {
        ...
        _authorizationLb.text = [NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"无法访问照片\n请点击这里前往设置中允许访问照片")];
    ...
    [self.manager FetchAllPhotoForPHFetchResult:model.result Index:model.index FetchResult:^(NSArray *photos, NSArray *videos, NSArray *Objs) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (model.albumName.length == 0) {
                model.albumName = iStoreLocalizedString(@"相机胶卷");
            }
...
- (void)goCameraVC {
    if (self.manager.goCamera) {
        ...
        if(![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            [self.view showImageHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"无法使用相机!")]];
            return;
        }
        AVAuthorizationStatus authStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
        if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"无法使用相机")] message:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"请在设置-隐私-相机中允许访问相机")] delegate:self cancelButtonTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"取消")] otherButtonTitles:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"设置")], nil];
            [alert show];
            return;
        }
 ...
 */
- (void)getObjs {
    [self.view showLoadingHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"加载中")]];
...
- (UINavigationItem *)navItem {
    if (!_navItem) {
        ...
        _navItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"取消")] style:UIBarButtonItemStylePlain target:self action:@selector(cancelClick)];
        ...
        [titleBtn setTitle:iStoreLocalizedString(@"相机胶卷") forState:UIControlStateNormal];
        [titleBtn addTarget:self action:@selector(pushAlbumList:) forControlEvents:UIControlEventTouchUpInside];
        self.titleBtn = titleBtn;
        _navItem.title = iStoreLocalizedString(@"相机胶卷");
...
- (void)setup {
    ...
    if (!self.manager.singleSelected) {
        ...
        if (self.manager.selectedList.count > 0) {
            ...
            [self.rightBtn setTitle:[NSString stringWithFormat:@"%@(%ld)",[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"下一步")],self.manager.selectedList.count] forState:UIControlStateNormal];
            ...
            [self.rightBtn setTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"下一步")] forState:UIControlStateNormal];
...
//    }
    ...
    if (!self.manager.singleSelected) {
        ...
        if (self.manager.selectedPhotos.count > 0 && self.manager.isOriginal) {
            ...
            [bottomView.originalBtn setTitle:[NSString stringWithFormat:@"%@(%@)",[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"原图")],self.manager.photosTotalBtyes] forState:UIControlStateNormal];
    ...
    if (self.manager.saveSystemAblum) {
        ...
        if (model.type == HXPhotoModelMediaTypeCameraPhoto) {
            [HXPhotoTools saveImageToAlbum:model.previewPhoto completion:^{
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [weakSelf.view showImageHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"照片保存失败!")]];
                });
            }];
        }else {
            [HXPhotoTools saveVideoToAlbum:model.videoURL completion:^{
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [weakSelf.view showImageHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"视频保存失败!")]];
                });
            }];
        }
        return;
...
- (void)changeOriginalState:(BOOL)selected IsChange:(BOOL)isChange
{
    if (selected) { // 选中时
        if (isChange) { // 改变成初始状态
            ...
            [self.bottomView.originalBtn setTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"原图")] forState:UIControlStateNormal];
        }
        // 记录原图按钮的frame
        ...
        [HXPhotoTools FetchPhotosBytes:self.manager.selectedPhotos completion:^(NSString *totalBytes) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                [weakSelf.bottomView.originalBtn setTitle:[NSString stringWithFormat:@"%@(%@)",[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"原图")],totalBytes] forState:UIControlStateNormal];
...
- (void)didNextClick:(UIButton *)button {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"newSelect" object:nil];
...
- (UIActivityIndicatorView *)indica {
    if (!_indica) {
        ...
        CGFloat indicaX = self.bottomView.originalBtn.titleLabel.frame.origin.x + [HXPhotoTools getTextWidth:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"原图")] height:self.bottomView.originalBtn.frame.size.height / 2 fontSize:14] + 5;
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        [_rightBtn setTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"下一步")] forState:UIControlStateNormal];
...
- (void)setup {
    ...
    [previewBtn setTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"预览")] forState:UIControlStateNormal];
    ...
    [originalBtn setTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"原图")] forState:UIControlStateNormal];
