...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = [NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"拍摄")];
...
- (void)cameraDistrict
{
    ...
    navItem.title = [NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"拍摄")];
...
- (void)didPhotoClick {
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"无法使用相机")] message:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"请在设置-隐私-相机中允许访问相机")] delegate:self cancelButtonTitle:nil otherButtonTitles:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"确定")], nil];
        [alert show];
        return;
    }
    AVCaptureConnection *conntion = [self.imageOutput connectionWithMediaType:AVMediaTypeVideo];
    if (!conntion) {
        [self.view showImageHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"拍摄失败")]];
        return;
    }
...
- (void)didLongClick:(UILongPressGestureRecognizer *)longRPG
{
    if ([longRPG state] == UIGestureRecognizerStateBegan) {
        ...
        if ([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeAudio] != AVAuthorizationStatusAuthorized) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"无法使用麦克风")] message:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"请在设置-隐私-相机中允许访问麦克风")] delegate:self cancelButtonTitle:nil otherButtonTitles:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"确定")], nil];
            [alert show];
            return;
        }
...
- (UIView *)moveView
{
    if (!_moveView) {
        ...
        [changePhotoBtn setTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"照片")] forState:UIControlStateNormal];
        ...
        [changeVideoBtn setTitle:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"视频")] forState:UIControlStateNormal];
...
- (void)nextClick
{
    ...
    if (self.changePhotoBtn.selected) {
        ...
        self.timer = nil;
        if (self.videoTime < 3) {
            [self.view showImageHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"录制时间少于3秒")]];
            return;
        }
        [self.playerLayer.player pause];
        __weak typeof(self) weakSelf = self;
        [self.view showLoadingHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"处理中")]];
        ...
        [self clipVideoCompleted:^{
            ...
            weakSelf.view.userInteractionEnabled = YES;
            [weakSelf.view handleLoading];
            [weakSelf.view showImageHUDText:[NSBundle hx_localizedStringForKey:iStoreLocalizedString(@"处理失败,请重试!")]];
        }];
