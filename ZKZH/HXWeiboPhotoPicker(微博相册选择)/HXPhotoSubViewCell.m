...
- (UIButton *)deleteBtn {
    if (!_deleteBtn) {
        ...
        [_deleteBtn setImage:[HXPhotoTools hx_imageNamed:iStoreLocalizedString(@"weChat_dele") ] forState:UIControlStateNormal];
...
- (void)didDeleteClick {
    if (self.model.networkPhotoUrl.length > 0) {
        if (self.showDeleteNetworkPhotoAlert) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:iStoreLocalizedString(@"提示") message:@"是否删除此照片" delegate:self cancelButtonTitle:iStoreLocalizedString(@"取消") otherButtonTitles:iStoreLocalizedString(@"确定"), nil];
            [alert show];
            return;
        }
    }
